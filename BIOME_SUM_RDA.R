...
SpARCTIC<-ARCTIC[which(ARCTIC$SEASON==2)*1,]

WEST<-GLOBAL[which(GLOBAL$DOMAIN==3)*1,]
SpWEST<-WEST[which(WEST$SEASON==2)*1,]

TRADE<-GLOBAL[which(GLOBAL$DOMAIN==4)*1,]
SpTRADE<-TRADE[which(TRADE$SEASON==2)*1,]

ANTARCTIC<-GLOBAL[which(GLOBAL$DOMAIN==5)*1,]
SpANT<-ANTARCTIC[which(ANTARCTIC$SEASON==2)*1,]
...
plot(pigment_rda, type="n", xlim = c(-1,1), ylim = c(-1,1),
     main = "Biome RDA Analysis Summer")
...
CB<-read.table(file="POLARSUM.csv",sep=",",h=T)
CC<-read.table(file="WESTSUM.csv",sep=",",h=T)
CD<-read.table(file="TRADESSUM.csv",sep=",",h=T)
