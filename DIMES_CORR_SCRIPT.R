...
#Read dataset
JC069<-read.table(file="DIMES_bottle_file.csv",sep=",",h=T)

#Create data frame of selected variables and ensure they are numeric
BFdata <- select(JC069, YEAR, MONTH, DAY, LONG, LAT, SST, DEPTH, PERID, FUCO, BUT, HEX, ZEA, DVCHLA, CHLA,DIAD,NO3_NO2,SILICATE,PHOS,Cd,Fe,Ni,Cu,Zn,Mn,Co)
...
df2 <- cbind(PERID_N, BUTA_N, FUCO_N, HEXA_N, ZEAX_N,DIAD_N,Pig_df$CHLA,Pig_df$SST,Pig_df$NO3_NO2,Pig_df$SILICATE,Pig_df$PHOS,Pig_df$Fe,Pig_df$Zn,Pig_df$Mn,Pig_df$Ni,Pig_df$Cd,Pig_df$Cu,Pig_df$Co)
colnames(df2)<-c("PERID","BUT19","FUCO","HEX19","ZEAX","DIAD","CHLA","TEMP","NO3_NO2","Silicate","Phos","Fe","Zn","Mn","Ni","Cd","Cu","Co")
