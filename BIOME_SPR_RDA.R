...
SpARCTIC<-ARCTIC[which(ARCTIC$SEASON==1)*1,]

WEST<-GLOBAL[which(GLOBAL$DOMAIN==3)*1,]
SpWEST<-WEST[which(WEST$SEASON==1)*1,]

TRADE<-GLOBAL[which(GLOBAL$DOMAIN==4)*1,]
SpTRADE<-TRADE[which(TRADE$SEASON==1)*1,]

ANTARCTIC<-GLOBAL[which(GLOBAL$DOMAIN==5)*1,]
SpANT<-ANTARCTIC[which(ANTARCTIC$SEASON==1)*1,]
...
plot(pigment_rda, type="n", xlim = c(-2,2), ylim = c(-2,2),
     main = "Biome RDA Analysis Spring")
...
CB<-read.table(file="POLARSPR.csv",sep=",",h=T)
CC<-read.table(file="WESTSPR.csv",sep=",",h=T)
CD<-read.table(file="TRADESSPR.csv",sep=",",h=T)
